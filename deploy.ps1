$grp = "rg-containerapps"
$loc = "northeurope"
$environment = "cne-dpr"
$STORAGE_ACCOUNT = "demodapralfortes"
...
# rebuild images
docker build -t alfogo/todoappbackend -f 'TodoApp.Backend\Dockerfile' .
docker push alfogo/todoappbackend

docker build -t alfogo/todoappfrontend -f 'TodoApp.Frontend\Dockerfile' .
docker push alfogo/todoappfrontend
...
az containerapp create `
  ...
  --image alfogo/todoappbackend:latest `
  --target-port 80 `
  --ingress 'internal' `
  --min-replicas 1 `
  --max-replicas 5 `
...
az containerapp create `
  ...
  --resource-group $grp `
  --environment $environment `
  --image alfogo/todoappfrontend:latest `
  --target-port 80 `
  --ingress 'external' `
  --min-replicas 0 `
  --max-replicas 5 `
